...
import type {
  AgentTaskDTO,
  AgentTaskEventDTO,
  AgentTaskTestResultDTO,
  CreateTaskPullRequestRequestDTO,
  GitChangedFileDTO,
  GitHubBranchProtectionDTO,
  GitHubCheckRunDTO,
  GitHubIssueDTO,
  GitHubMergeReadinessDTO,
  GitHubPullRequestCommentDTO,
  GitHubPullRequestDTO,
  GitHubPullRequestReviewDTO,
  GitHubRepoRefDTO,
  GitHubReviewEvent,
  TaskBranchCleanupDTO,
  ImportedGitHubIssueDraftDTO,
  ImportGitHubIssueRequestDTO,
  MergeTaskPullRequestRequestDTO,
  RefreshAgentTaskRequestDTO,
  ReviewTaskPullRequestRequestDTO,
  RunSessionDTO,
...
type TaskListener = (task: AgentTaskDTO) => void;
type TaskEventListener = (event: AgentTaskEventDTO) => void;

export interface PersistedTaskRecord {
  summary: AgentTaskDTO;
  events: AgentTaskEventDTO[];
}

interface InternalTask extends PersistedTaskRecord {
...
export class TaskManager {
  ...
  private readonly eventListeners = new Set<TaskEventListener>();

  constructor(
    private readonly runManager: RunManager,
    restoredTasks: PersistedTaskRecord[] = []
  ) {
  ...
  listTasks(): AgentTaskDTO[] {
    return Array.from(this.tasks.values())
      .map((entry) => entry.summary)
      .sort((left, right) => right.createdAtMs - left.createdAtMs);
  }

  getTask(taskId: string): AgentTaskDTO | null {
    return this.tasks.get(taskId)?.summary ?? null;
  }

  getEvents(taskId: string): AgentTaskEventDTO[] {
    return this.tasks.get(taskId)?.events ?? [];
  }

  listPersistedTasks(): PersistedTaskRecord[] {
    return Array.from(this.tasks.values())
      .map((entry) => ({
  ...
  async importGitHubIssueDraft(
    request: ImportGitHubIssueRequestDTO
  ): Promise<ImportedGitHubIssueDraftDTO> {
  ...
  async startTask(request: StartAgentTaskRequestDTO): Promise<AgentTaskDTO> {
    const git = await getGitRepoContext(request.cwd);
    if (git.dirty) {
      throw new Error(
        'The git working tree has local changes. Commit or stash them before starting a task.'
      );
    }
    ...
    const title =
      request.title?.trim() || sourceIssue?.title || summarizeTaskTitle(effectivePrompt);
    ...
    try {
      ...
      this.appendEvent(
        ...
        'system',
        'success',
        `Assigned to ${request.provider}. Run ${run.id.slice(0, 8)} started.`
      );
    } catch (error) {
  ...
  stopTask(taskId: string): AgentTaskDTO | null {
  ...
  async refreshTask(
    taskId: string,
    request: RefreshAgentTaskRequestDTO = {}
    ...
    if (!task.refreshPromise) {
      task.refreshPromise = this.runRefresh(taskId, request).finally(() => {
        ...
        if (current) current.refreshPromise = undefined;
      });
    }
  ...
  async createPullRequest(
    taskId: string,
    request: CreateTaskPullRequestRequestDTO
  ): Promise<AgentTaskDTO> {
    const task = this.tasks.get(taskId);
    if (!task) {
      throw new Error('Task not found.');
    ...
    const aheadCount = await getAheadCount(
      task.summary.repoRoot,
      task.summary.baseBranch,
      task.summary.branchName
    );
    ...
    const aheadAfterCommit = await getAheadCount(
      task.summary.repoRoot,
      task.summary.baseBranch,
      task.summary.branchName
    );
    ...
    const response = await githubRequest<{
      ...
      github,
      `/repos/${github.owner}/${github.name}/pulls`,
      {
        method: 'POST',
        body: {
  ...
  async reviewPullRequest(
    taskId: string,
    request: ReviewTaskPullRequestRequestDTO
  ): Promise<AgentTaskDTO> {
    const task = this.tasks.get(taskId);
    if (!task) {
      throw new Error('Task not found.');
    ...
    await githubRequest(
      github,
      `/repos/${github.owner}/${github.name}/pulls/${pullRequest.number}/reviews`,
      {
        method: 'POST',
        body: {
          event: request.event,
    ...
    this.appendEvent(
      ...
      'github',
      'success',
      `${reviewEventLabel(request.event)} submitted on pull request #${pullRequest.number}.`
    );
    await this.refreshTask(taskId);
    return this.tasks.get(taskId)!.summary;
  }
  ...
  async mergePullRequest(
    taskId: string,
    request: MergeTaskPullRequestRequestDTO
  ): Promise<AgentTaskDTO> {
    const task = this.tasks.get(taskId);
    if (!task) {
      throw new Error('Task not found.');
    ...
    await githubRequest(
      github,
      `/repos/${github.owner}/${github.name}/pulls/${pullRequest.number}/merge`,
      {
        method: 'PUT',
        body: {
    ...
    this.appendEvent(
      ...
      'github',
      'success',
      `Merged pull request #${pullRequest.number} with ${(request.method ?? 'squash')} strategy.`
    );
    const cleanup = await cleanupMergedTaskBranch(
      task.summary.repoRoot,
      task.summary.baseBranch,
      task.summary.branchName
    );
    ...
    this.appendEvent(
      taskId,
      'git',
      cleanup.localBranchDeleted ? 'success' : 'info',
      cleanup.message ?? 'Merge cleanup finished.'
    );
    return this.tasks.get(taskId)!.summary;
  }

  private async handleRunUpdate(run: RunSessionDTO): Promise<void> {
    for (const task of this.tasks.values()) {
      if (task.summary.runId !== run.id) continue;
      ...
      if (run.status === 'failed') {
        ...
        this.appendEvent(
          ...
          'system',
          'error',
          run.error ? `Agent run failed: ${run.error}` : 'Agent run failed.'
        );
        await this.refreshTask(task.summary.id, { rerunTests: false });
        continue;
      }
  ...
  private async runRefresh(
    ...
    this.updateTask(taskId, {
      ...
      status: nextStatus,
      endedAtMs:
        nextStatus === 'ready' || nextStatus === 'failed' || nextStatus === 'stopped'
          ? task.summary.endedAtMs ?? Date.now()
          : task.summary.endedAtMs,
      lastRefreshedAtMs: Date.now(),
      github: {
        ...repoSummary.github,
        compareUrl: buildGitHubCompareUrl(
          repoSummary.github,
          task.summary.baseBranch,
          task.summary.branchName
        ),
    ...
    this.appendEvent(
      ...
      'git',
      'success',
      repoSummary.workingTreeStatus === 'clean'
        ? 'Repository is clean after refresh.'
        : `Detected ${repoSummary.changedFiles.length} changed file(s).`
    );
  }

  private async runTaskTests(task: AgentTaskDTO): Promise<AgentTaskTestResultDTO> {
    ...
    const next: AgentTaskTestResultDTO = {
      command,
    ...
    this.appendEvent(
      ...
      result.exitCode === 0 ? 'Tests passed.' : `Tests failed with code ${result.exitCode}.`
    );
    return next;
  }

  private updateTask(taskId: string, patch: Partial<AgentTaskDTO>): void {
    const task = this.tasks.get(taskId);
    if (!task) return;
  ...
  private appendEvent(
    taskId: string,
    kind: AgentTaskEventDTO['kind'],
    tone: AgentTaskEventDTO['tone'],
    message: string
  ): void {
    const task = this.tasks.get(taskId);
    if (!task) return;

    const event: AgentTaskEventDTO = {
      id: randomUUID(),
      taskId,
      kind,
      tone,
  ...
  private emitTask(task: AgentTaskDTO): void {
    for (const listener of this.taskListeners) {
      listener(task);
    }
  }

  private restore(restoredTasks: PersistedTaskRecord[]): void {
    ...
    for (const entry of restoredTasks) {
      ...
      if (summary.status === 'stopped' && wasActiveTask(entry.summary.status)) {
        events.push({
          ...
          tone: 'info',
          message:
            '[cli-run-ui] Restored after server restart. The original task run is no longer attached.',
...
async function inspectTaskRepo(
  task: AgentTaskDTO
): Promise<
  Pick<
    AgentTaskDTO,
...
> {
  ...
  const checks =
    github.connected && pullRequest?.headSha
...
function deriveTaskStatus(task: AgentTaskDTO, testResult: AgentTaskTestResultDTO): AgentTaskDTO['status'] {
  if (task.status === 'merged' || task.status === 'stopped') return task.status;
  if (task.error) return 'failed';
  if (testResult.status === 'failed') return 'failed';
  return 'ready';
}

function normalizeRestoredTask(task: AgentTaskDTO, restoredAtMs: number): AgentTaskDTO {
...
function wasActiveTask(status: AgentTaskDTO['status']) {
  return status === 'preparing' || status === 'running';
}

async function getGitRepoContext(cwd: string): Promise<GitRepoContext> {
...
async function refreshGitHubContext(
  cwd: string,
  current: GitHubRepoRefDTO
): Promise<GitHubRepoRefDTO> {
...
async function checkoutTaskBranch(repoRoot: string, baseBranch: string, branchName: string) {
  await git(repoRoot, ['checkout', baseBranch]);
  await git(repoRoot, ['checkout', '-b', branchName]);
}

async function ensureBranchCheckedOut(repoRoot: string, branchName: string) {
  const currentBranch = (await git(repoRoot, ['branch', '--show-current'])).trim();
  if (currentBranch === branchName) return;
  const dirty = await hasWorkingTreeChanges(repoRoot);
  if (dirty) {
    throw new Error(
      `The working tree is dirty while ${currentBranch} is checked out. Switch back to ${branchName} after committing or stashing local changes.`
    );
  }
...
async function hasStagedDiff(cwd: string) {
  const diff = await gitOptional(cwd, ['diff', '--cached', '--name-only']);
  return Boolean(diff?.trim());
}

async function getAheadCount(cwd: string, baseBranch: string, branchName: string) {
  const value = await gitOptional(cwd, ['rev-list', '--count', `${baseBranch}..${branchName}`]);
  return Number.parseInt(value?.trim() || '0', 10) || 0;
}

async function listChangedFiles(cwd: string, baseBranch: string): Promise<GitChangedFileDTO[]> {
  const workingTree = (await gitOptional(cwd, ['status', '--porcelain=v1'])).trim();
  const committed = (
    await gitOptional(cwd, ['diff', '--name-status', '--find-renames', `${baseBranch}...HEAD`])
  ).trim();

  const merged = new Map<string, GitChangedFileDTO>();
...
function parseWorkingTreeFiles(output: string): GitChangedFileDTO[] {
  if (!output) return [];
  ...
  return output
    ...
    .map((line) => {
      if (line.startsWith('?? ')) {
        return { path: line.slice(3).trim(), status: 'untracked' as const };
      }
...
function parseNameStatusFiles(output: string): GitChangedFileDTO[] {
  if (!output) return [];
  ...
  return output
    ...
    .map((line) => {
      ...
      if (!statusCode || !first) {
        return { path: line, status: 'unknown' as const };
      }
...
function mapGitStatusCode(code: string): GitChangedFileDTO['status'] {
...
async function buildDiffStat(cwd: string, baseBranch: string) {
  const committed = (
    await gitOptional(cwd, ['diff', '--stat', '--find-renames', `${baseBranch}...HEAD`])
  ).trim();
  const workingTree = (await gitOptional(cwd, ['diff', '--stat', '--find-renames'])).trim();
  return combineDiffSections(committed, workingTree);
}

async function buildDiffExcerpt(cwd: string, baseBranch: string) {
  const committed = (
    await gitOptional(cwd, [
      'diff',
      '--no-ext-diff',
      '--find-renames',
      '--unified=1',
  ...
  const workingTree = (
    await gitOptional(cwd, ['diff', '--no-ext-diff', '--find-renames', '--unified=1'])
  ).trim();
...
function combineDiffSections(committed: string, workingTree: string) {
  const parts: string[] = [];
  if (committed) {
...
function summarizeTaskTitle(prompt: string) {
  const normalized = prompt.replace(/\s+/g, ' ').trim();
  return normalized.slice(0, 80) || 'Agent task';
}

function normalizeTaskBranchName(branchName: string | undefined, title: string) {
...
function parseGitHubRemote(remoteUrl: string): GitHubRemoteRef | null {
...
async function resolveGitHubIssue(issueUrl: string): Promise<GitHubIssueDTO> {
...
function parseGitHubIssueUrl(value: string): GitHubIssueLocator | null {
  try {
    ...
    if (!parts[0] || !parts[1] || !Number.isFinite(issueNumber)) return null;
    return {
      owner: parts[0],
      name: parts[1],
      number: issueNumber,
...
): Promise<GitHubPullRequestDTO | undefined> {
...
function buildIssuePrompt(issue: GitHubIssueDTO) {
  const lines = [
  ...
  lines.push(
    '',
    'Please implement the change in the current repository, explain the approach briefly, and leave the branch ready for review.'
  );
  return lines.join('\n');
}
...
async function fetchPullRequestReviews(
  github: GitHubRepoRefDTO,
  pullRequestNumber: number
): Promise<GitHubPullRequestReviewDTO[]> {
  if (!github.connected || github.provider !== 'github' || !github.owner || !github.name) {
    return [];
  }
  const token = readGitHubToken();
  const response = await fetch(
    `${resolveGitHubApiBaseUrl()}/repos/${github.owner}/${github.name}/pulls/${pullRequestNumber}/reviews`,
    {
      headers: {
        Accept: 'application/vnd.github+json',
        ...(token ? { Authorization: `Bearer ${token}` } : {}),
...
async function fetchPullRequestComments(
  ...
  const token = readGitHubToken();
  const response = await fetch(
    `${resolveGitHubApiBaseUrl()}/repos/${github.owner}/${github.name}/issues/${pullRequestNumber}/comments`,
    {
      headers: {
        Accept: 'application/vnd.github+json',
        ...(token ? { Authorization: `Bearer ${token}` } : {}),
...
async function fetchCheckRuns(
  github: GitHubRepoRefDTO,
  ref: string
): Promise<GitHubCheckRunDTO[]> {
  if (!github.connected || github.provider !== 'github' || !github.owner || !github.name) {
    return [];
  }
  const token = readGitHubToken();
  const response = await fetch(
    `${resolveGitHubApiBaseUrl()}/repos/${github.owner}/${github.name}/commits/${ref}/check-runs`,
    {
      headers: {
        Accept: 'application/vnd.github+json',
        ...(token ? { Authorization: `Bearer ${token}` } : {}),
...
async function fetchBranchProtection(
  github: GitHubRepoRefDTO,
  branch: string
): Promise<GitHubBranchProtectionDTO> {
  if (!github.connected || github.provider !== 'github' || !github.owner || !github.name) {
    return {
      enabled: false,
      requiredCheckContexts: [],
  ...
  const response = await fetch(
    `${resolveGitHubApiBaseUrl()}/repos/${github.owner}/${github.name}/branches/${encodeURIComponent(branch)}/protection`,
    {
      headers: {
        Accept: 'application/vnd.github+json',
  ...
  return {
    ...
    requiredApprovingReviewCount:
      payload.required_pull_request_reviews?.required_approving_review_count ?? undefined,
    dismissesStaleReviews:
      payload.required_pull_request_reviews?.dismiss_stale_reviews ?? undefined,
    requiresConversationResolution:
      payload.required_conversation_resolution?.enabled ?? undefined,
...
function buildMergeReadiness(
  ...
  reviews: GitHubPullRequestReviewDTO[],
  ...
  if (branchProtection?.requiredApprovingReviewCount) {
    if (approvals < branchProtection.requiredApprovingReviewCount) {
      reasons.push(
        `Requires ${branchProtection.requiredApprovingReviewCount} approval(s); only ${approvals} recorded.`
      );
    }
  }

  const failedChecks = checks.filter(
    (check) => check.status === 'completed' && check.conclusion && !['success', 'neutral', 'skipped'].includes(check.conclusion)
  );
  ...
  if (branchProtection?.requiredCheckContexts.length) {
    ...
    const missingRequiredChecks = branchProtection.requiredCheckContexts.filter(
      (context) => !checkNames.has(context)
    );
...
async function cleanupMergedTaskBranch(
  repoRoot: string,
  baseBranch: string,
  branchName: string
): Promise<TaskBranchCleanupDTO> {
...
function buildGitHubCompareUrl(
  github: {
    ...
    name: string;
    connected?: boolean;
    provider?: GitHubRepoRefDTO['provider'];
  },
  baseBranch: string,
  branchName: string
) {
...
function ensureGitHubReady(github: GitHubRepoRefDTO) {
  if (!github.connected || github.provider !== 'github' || !github.owner || !github.name) {
  ...
  if (!token) {
    throw new Error(
      'Set CLI_RUN_UI_GITHUB_TOKEN (or GITHUB_TOKEN) to enable one-click pull request, review, and merge actions.'
    );
  }
  return {
    owner: github.owner,
...
async function githubRequest<T>(
  github: { owner: string; name: string; token: string },
  pathname: string,
...
function buildPullRequestBody(task: AgentTaskDTO) {
  const lines = [
    '## Summary',
    '',
    `- Task: ${task.title}`,
  ...
  if (task.testResult.command) {
    lines.push(
      `- Tests: ${task.testResult.status === 'passed' ? 'passed' : task.testResult.status}`
    );
  }
  ...
  if (task.sourceIssue) {
    lines.push(
      ...
      '## Linked Issue',
      '',
      `Closes #${task.sourceIssue.number}`,
      '',
      `Source: ${task.sourceIssue.url}`
    );
  }
...
function reviewEventLabel(event: GitHubReviewEvent) {
  if (event === 'APPROVE') return 'Approval';
  if (event === 'REQUEST_CHANGES') return 'Request changes';
  return 'Review comment';
}

function truncateText(value: string, limit = MAX_TEST_OUTPUT_CHARS) {
