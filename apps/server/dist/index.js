...
const runtimePersistence = createRuntimePersistenceTask(historyStore, runManager, taskManager, terminalManager, relayManager);
watcherHub.start();
...
function createRuntimePersistenceTask(store, runs, tasks, terminals, relays) {
    let timer = null;
    let writing = false;
    let queued = false;
    const persist = async () => {
