import fs from 'node:fs';
import path from 'node:path';
import { expandHome, basenameSafe } from '../../utils/path.js';
import { parseTimeMs } from '../../utils/time.js';
import { safeJsonParse } from '../../utils/json.js';
export class CodexIndex {
    rootDir;
    sessionsDir;
    sessionIdToFilePath = new Map();
    filePathToSessionId = new Map();
    sessionMeta = new Map();
    built = false;
    constructor(options) {
        this.rootDir = expandHome(options?.rootDir ?? '~/.codex');
        this.sessionsDir = path.join(this.rootDir, 'sessions');
    }
    getRootDir() {
        return this.rootDir;
    }
    async build(force = false) {
        if (this.built && !force)
            return;
        this.built = true;
        await this.scanSessions();
    }
    listSessionIds() {
        return Array.from(this.sessionIdToFilePath.keys());
    }
    getFilePath(sessionId) {
        return this.sessionIdToFilePath.get(sessionId);
    }
    getMeta(sessionId) {
        return this.sessionMeta.get(sessionId);
    }
    getSessionIdByFile(filePath) {
        return this.filePathToSessionId.get(filePath);
    }
    refreshFileSync(filePath) {
        const meta = readSessionMetaSync(filePath);
        if (!meta)
            return null;
        const sessionId = meta.sessionId;
        this.sessionIdToFilePath.set(sessionId, filePath);
        this.filePathToSessionId.set(filePath, sessionId);
        this.sessionMeta.set(sessionId, meta);
        return meta;
    }
    async refreshFile(filePath) {
        const meta = await readSessionMeta(filePath);
        if (!meta)
            return;
        const sessionId = meta.sessionId;
        this.sessionIdToFilePath.set(sessionId, filePath);
        this.filePathToSessionId.set(filePath, sessionId);
        this.sessionMeta.set(sessionId, meta);
    }
    async scanSessions() {
        this.sessionIdToFilePath.clear();
        this.filePathToSessionId.clear();
        this.sessionMeta.clear();
        if (!fs.existsSync(this.sessionsDir))
            return;
        const stack = [this.sessionsDir];
        while (stack.length > 0) {
            const current = stack.pop();
            if (!current)
                continue;
            const entries = await fs.promises.readdir(current, { withFileTypes: true });
            for (const entry of entries) {
                const fullPath = path.join(current, entry.name);
                if (entry.isDirectory()) {
                    stack.push(fullPath);
                    continue;
                }
                if (!entry.isFile())
                    continue;
                if (!entry.name.startsWith('rollout-') || !entry.name.endsWith('.jsonl'))
                    continue;
                const meta = await readSessionMeta(fullPath);
                if (!meta)
                    continue;
                this.sessionIdToFilePath.set(meta.sessionId, fullPath);
                this.filePathToSessionId.set(fullPath, meta.sessionId);
                this.sessionMeta.set(meta.sessionId, meta);
            }
        }
    }
}
async function readSessionMeta(filePath) {
    const lines = await readFirstLines(filePath, 16384);
    for (const line of lines) {
        const parsed = safeJsonParse(line);
        if (!parsed)
            continue;
        if (parsed.type !== 'session_meta')
            continue;
        const payload = parsed.payload;
        if (!payload)
            continue;
        const sessionId = typeof payload.id === 'string' ? payload.id : undefined;
        if (!sessionId)
            continue;
        const cwd = typeof payload.cwd === 'string' ? payload.cwd : undefined;
        const startedAt = parseTimeMs(payload.timestamp) ??
            parseTimeMs(parsed.timestamp) ??
            parseTimeMs(payload.started_at) ??
            undefined;
        return {
            sessionId,
            cwd,
            startedAtMs: startedAt,
            filePath,
        };
    }
    return null;
}
function readSessionMetaSync(filePath) {
    const lines = readFirstLinesSync(filePath, 16384);
    for (const line of lines) {
        const parsed = safeJsonParse(line);
        if (!parsed)
            continue;
        if (parsed.type !== 'session_meta')
            continue;
        const payload = parsed.payload;
        if (!payload)
            continue;
        const sessionId = typeof payload.id === 'string' ? payload.id : undefined;
        if (!sessionId)
            continue;
        const cwd = typeof payload.cwd === 'string' ? payload.cwd : undefined;
        const startedAt = parseTimeMs(payload.timestamp) ??
            parseTimeMs(parsed.timestamp) ??
            parseTimeMs(payload.started_at) ??
            undefined;
        return {
            sessionId,
            cwd,
            startedAtMs: startedAt,
            filePath,
        };
    }
    return null;
}
async function readFirstLines(filePath, maxBytes) {
    const stream = fs.createReadStream(filePath, { start: 0, end: maxBytes });
    const chunks = [];
    await new Promise((resolve, reject) => {
        stream.on('data', (chunk) => chunks.push(Buffer.from(chunk)));
        stream.on('error', (err) => reject(err));
        stream.on('end', () => resolve());
    });
    const content = Buffer.concat(chunks).toString('utf8');
    return content
        .split(/\r?\n/)
        .map((line) => line.trim())
        .filter((line) => line.length > 0);
}
function readFirstLinesSync(filePath, maxBytes) {
    try {
        const fd = fs.openSync(filePath, 'r');
        const buffer = Buffer.allocUnsafe(maxBytes);
        const bytesRead = fs.readSync(fd, buffer, 0, maxBytes, 0);
        fs.closeSync(fd);
        const content = buffer.subarray(0, bytesRead).toString('utf8');
        return content
            .split(/\r?\n/)
            .map((line) => line.trim())
            .filter((line) => line.length > 0);
    }
    catch {
        return [];
    }
}
export function toProjectNameFromCwd(cwd) {
    return basenameSafe(cwd);
}
//# sourceMappingURL=CodexIndex.js.map